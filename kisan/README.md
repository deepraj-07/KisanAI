...
│   ├── advice/route.ts           # Unified advice endpoint (weather + market + AI)
│   ├── gemini/route.ts           # AI handler (diagnosis/advisor)
...
The app currently uses Gemini models in `app/api/gemini/route.ts`:

| Model | Tier | Rate Limit | Best For |
|---|---|---|---|
| `gpt-4.1-mini-2025-04-14` | Paid | Account-based | Current default |

```typescript
// app/api/gemini/route.ts — line 28
const MODEL_TEXT   = "gpt-4.1-mini-2025-04-14"; // change here
const MODEL_VISION = "gpt-4.1-mini-2025-04-14"; // and here
```
...
```
→ Rate limit hit. Wait and retry, or rotate to another Gemini key/project if needed.

### AI Model 404 Error
```
The model is not found
```
→ Ensure `app/api/gemini/route.ts` and `app/api/advisory/route.ts` use an available model.
...
│                                                                  │
│  /api/advisory  →  Unified advisory API (weather + market + AI)│
│  /api/gemini  →  AI analysis API (chat/vision/crop advisor)    │
